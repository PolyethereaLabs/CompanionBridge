...
from models import ProcessingSession, AnalyticsMetric, AdminUser
...
@app.route('/admin-access-portal', methods=['GET', 'POST'])
def admin_login():
...
def admin_dashboard():
    ...
    recent_sessions = ProcessingSession.query.order_by(desc(ProcessingSession.created_at)).limit(20).all()
    
    return render_template('admin_dashboard.html', analytics=analytics, recent_sessions=recent_sessions)
...
def calculate_analytics():
    ...
    total_conversations = db.session.query(func.sum(ProcessingSession.total_conversations)).scalar() or 0
    
    # Processing time average
    avg_processing_time = db.session.query(func.avg(ProcessingSession.processing_time)).scalar() or 0
    
    # Error rate in last 24 hours
    last_24h = datetime.utcnow() - timedelta(hours=24)
    recent_sessions = ProcessingSession.query.filter(ProcessingSession.created_at >= last_24h).count()
    recent_errors = ProcessingSession.query.filter(
        ProcessingSession.created_at >= last_24h,
        ProcessingSession.status == 'error'
    ).count()
    error_rate = (recent_errors / recent_sessions * 100) if recent_sessions > 0 else 0
    
    # Active sessions
    active_sessions = ProcessingSession.query.filter_by(status='processing').count()
    
    # Memory anchor success rate
    sessions_with_anchors = ProcessingSession.query.filter(ProcessingSession.memory_anchors_found > 0).count()
    memory_anchor_success_rate = (sessions_with_anchors / total_sessions * 100) if total_sessions > 0 else 0
    avg_memory_anchors = db.session.query(func.avg(ProcessingSession.memory_anchors_found)).scalar() or 0
    ...
    for i in range(6, -1, -1):
        ...
        count = ProcessingSession.query.filter(
            ProcessingSession.created_at >= day_start,
            ProcessingSession.created_at < day_end
        ).count()
    ...
    full_downloads = ProcessingSession.query.filter_by(full_downloaded=True).count()
    condensed_downloads = ProcessingSession.query.filter_by(condensed_downloaded=True).count()
    both_downloads = ProcessingSession.query.filter(
        ProcessingSession.full_downloaded == True,
        ProcessingSession.condensed_downloaded == True
    ).count()
    
    format_data = [
        full_downloads - both_downloads,  # Full only
        condensed_downloads - both_downloads,  # Condensed only
        both_downloads  # Both
    ]
    
    # Popular companion names
    companion_names = db.session.query(
        ProcessingSession.companion_name,
        func.count(ProcessingSession.companion_name).label('count')
    ).group_by(ProcessingSession.companion_name).order_by(desc('count')).limit(10).all()
    
    popular_companion_names = []
    for name, count in companion_names:
        if name:  # Skip None values
            percentage = (count / total_sessions * 100) if total_sessions > 0 else 0
            popular_companion_names.append({
                'name': name,
                'count': count,
    ...
    for range_name, min_size, max_size in file_size_ranges:
        if max_size == float('inf'):
            sessions = ProcessingSession.query.filter(ProcessingSession.file_size >= min_size)
        else:
            sessions = ProcessingSession.query.filter(
                ProcessingSession.file_size >= min_size,
                ProcessingSession.file_size < max_size
            )
        
        count = sessions.count()
        avg_time = sessions.with_entities(func.avg(ProcessingSession.processing_time)).scalar() or 0
    ...
    return {
        ...
        'memory_anchor_success_rate': memory_anchor_success_rate,
        'avg_memory_anchors': round(avg_memory_anchors, 1),
        'daily_data': daily_data,
        'daily_labels': daily_labels,
        'format_data': format_data,
        'popular_companion_names': popular_companion_names,
